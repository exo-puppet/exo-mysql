...
#   If InnoDB runs out of memory in this pool, it starts to allocate memory from the operating system and writes warning messages to
#   the MySQL error log.
#
#   For more informations, read :
...
#   this parameter should be set to :
#   * 0 => the log buffer is written out to the log file once per second and the flush to disk operation is performed on the log
#   file, but nothing is done at a transaction commit.
#   * 1 => the log buffer is written out to the log file at each transaction commit and the flush to disk operation is performed on
#   the log file.
#   * 2 => the log buffer is written out to the file at each commit, but the flush to disk operation is not performed on it.
#   However, the flushing on the log file takes place once per second also when the value is 2. Note that the once-per-second
#   flushing is not 100% guaranteed to happen every second, due to process scheduling issues.
#
#   For more informations, read :
...
#   The default value is 5MB. Sensible values range from 1MB to 1/N-th of the size of the buffer pool, where N is the number of log
#   files in the group.
...
#   innodb_data_home_dir to each path specified here.
#   The file sizes are specified in KB, MB, or GB (1024MB) by appending K, M, or G to the size value. The sum of the sizes of the
#   files must be at least 10MB.
#   If you do not specify innodb_data_file_path, the default behavior is to create a single 10MB auto-extending data file named
#   ibdata1.
#   The size limit of individual files is determined by your operating system.
#   You can set the file size to more than 4GB on those operating systems that support big files. You can also use raw disk
#   partitions as data files.
#
...
#   reopened), then you should increase the value of the table_open_cache variable.
...
#
#   The maximum size of internal in-memory temporary tables. (The actual limit is determined as the minimum of tmp_table_size and
#   max_heap_table_size.)
#   If an in-memory temporary table exceeds the limit, MySQL automatically converts it to an on-disk MyISAM table.
#   Increase the value of tmp_table_size (and max_heap_table_size if necessary) if you do many advanced GROUP BY queries and you
#   have lots of memory.
#   This variable does not apply to user-created MEMORY tables.
#
#   You can compare the number of internal on-disk temporary tables created to the total number of internal temporary tables created
#   by comparing the values of the Created_tmp_disk_tables and Created_tmp_tables variables.
#
#   For more informations, read :
...
#   (OPTIONAL) (default: 8)
#
#   How many threads the server should cache for reuse. When a client disconnects, the client's threads are put in the cache if
#   there are fewer than thread_cache_size threads there.
#   Requests for threads are satisfied by reusing threads taken from the cache if possible, and only when the cache is empty is a
...
#   However, if your server sees hundreds of connections per second you should normally set thread_cache_size high enough so that
#   most new connections use cached threads.
#   By examining the difference between the Connections and Threads_created status variables, you can see how efficient the thread
#   cache is.
#
#   For more informations, read :
...
#   (OPTIONAL) (default: 16M)
#
#   The amount of memory allocated for caching query results. The permissible values are multiples of 1024; other values are rounded
...
#   (OPTIONAL) (default: 2M)
#
#   Each session that needs to do a sort allocates a buffer of this size. sort_buffer_size is not specific to any storage engine and
#   applies in a general manner for optimization.
#   If you see many Sort_merge_passes per second in SHOW GLOBAL STATUS output, you can consider increasing the sort_buffer_size
#   value to speed up ORDER BY or GROUP BY operations that cannot be improved with query optimization or improved indexing.
#   The entire buffer is allocated even if it is not all needed, so setting it larger than required globally will slow down most
#   queries that sort. It is best to increase it as a session setting, and only for the sessions that need a larger size.
#
#   For more informations, read :
...
#   Index blocks for MyISAM tables are buffered and are shared by all threads. key_buffer_size is the size of the buffer used for
#   index blocks. The key buffer is also known as the key cache.
#   You can increase the value to get better index handling for all reads and multiple writes; on a system whose primary function is
#   to run MySQL using the MyISAM storage engine, 25% of the machine's total memory is an acceptable value for this variable.
...
#   This is because MySQL relies on the operating system to perform file system caching for data reads, so you must leave some room
...
#   One join buffer is allocated for each full join between two tables. For a complex join between several tables for which indexes
#   are not used, multiple join buffers might be necessary.
#   There is no gain from setting the buffer larger than required to hold each matching row, and all joins allocate at least the
#   minimum size, so use caution in setting this variable to a large value globally.
#   It is better to keep the global setting small and change to a larger setting only in sessions that are doing large joins.
#   Memory allocation time can cause substantial performance drops if the global size is larger than needed by most queries that use
#   it.
#
#   For more informations, read :
#   * http://dev.mysql.com/doc/refman/5.1/en/innodb-parameters.html#sysvar_innodb_rollback_on_timeout
...
#   This variable sets the maximum size to which user-created MEMORY tables are permitted to grow. The value of the variable is used
#   to calculate MEMORY table MAX_ROWS values.
#
#   For more informations, read :
...
#   You must increase this value if you are using large BLOB columns or long strings. It should be as big as the largest BLOB you
#   want to use.
#   The protocol limit for max_allowed_packet is 1GB. The value should be a multiple of 1024; nonmultiples are rounded down to the
#   nearest multiple.
#
#   For more informations, read :
